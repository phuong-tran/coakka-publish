...
listed below. npm `2.1.1` and NuGet `2.1.0` are published and expose file-lane.
PyPI retains its older registry version until its separately authenticated
publication completes; that immutable older package does not expose the
file-lane ABI.
