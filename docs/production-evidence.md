...
| Package-manager execution | Exact PyPI `1.4.6` and NuGet `1.4.7` retain their recorded registry execution. Go `v1.5.0` packages runtime `2.1.0+60ddf70d` and passes packaged request/reply on macOS ARM64 and Linux ARM64/x86-64; Swift `v2.1.0` passes Swift build, tests, runtime, transport, and packaged-consumer smoke on macOS ARM64. Known matching-host gaps remain explicit. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
