...
| Artifact source addon family | Eleven native addons at `1.1.0+d1032f6d` pass strict warnings, static analysis, Linux ASan/LSan, UBSan and TSan, internal protocol fixtures, public C ABI execution, bounded owner stress/repetition, installed export/dependency audits, deterministic packaging, and matching-host gates for every declared platform. Local Drop is intentionally POSIX-only. Tests use deterministic protocol peers; no live cloud-account certification or performance SLA is claimed. | Run each archive through `scripts/verify-runtime-addon-release.py`; see `runtime-addons/`. |
| SFTP artifact publisher addon | Replacement native addon `1.2.0+88b9a047` loads beside Runtime `2.3.0` on Linux ARM64/x86-64, macOS ARM64, and Windows 11 ARM64/x86-64. Every module exposes exactly 11 reviewed C symbols, has no ambient libssh2/libcrypto/libssl dependency, and passes pinned SFTP acquisition, integrity/no-clobber, resource recovery, failure, cancellation, and File Lane delivery evidence. | Verify its archive with `scripts/verify-runtime-addon-release.py`; the C11 sample pin must name the replacement coordinate. |
