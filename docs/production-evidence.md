...
| Package-manager execution | npm Node.js and Bun `2.1.1` clean installs pass native request/reply; Node route-miss deadletter and Electron main-process intent also pass. NuGet `2.1.0` passes repository-signature verification and clean packaged request/reply/deadletter; PyPI `1.4.6` retains its recorded registry execution. Go `v1.5.0` passes packaged request/reply on macOS ARM64 and Linux ARM64/x86-64; Swift `v2.1.0` passes its macOS ARM64 package smokes. | [Runtime Package And Platform Evidence](runtime-package-platform-evidence.md) |
