...
published and clean-install verified. NuGet `2.1.0` is also published and
clean-install verified. PyPI retains its earlier coordinate until its 2.1.0
upload and clean-registry install complete:
...
against the PyPI 1.4.x package. Use an exact coordinate whose
release receipt records native `2.1.0+60ddf70d` or a compatible successor.
