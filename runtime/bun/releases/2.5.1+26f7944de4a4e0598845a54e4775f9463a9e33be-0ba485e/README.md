...
Replica File and mailbox Stream paths use `openOwned(...)`,
`prepareReceiveGrant(...)` or `preparePublishGrant(...)`, and the grant's
`toSendSpec(...)` or `toSubscribeSpec(...)`. Grant JSON retains the bounded
mailbox frame count. Run waits/mailbox loops outside Bun's main event loop and
follow the [owner-grant ONE/ALL sample](https://github.com/phuong-tran/coakka-samples/blob/main/docs/runtime-lane-owner-grants.md).
