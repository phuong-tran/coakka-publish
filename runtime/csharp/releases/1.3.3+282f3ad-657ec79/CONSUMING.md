...
The current NuGet-ready candidate is `CoAkka.Runtime` version `1.3.4`, bundled
with native runtime generation `1.3.3+282f3ad`.
...
dotnet add package CoAkka.Runtime --version 1.3.4 --source coakka-local
```

After the NuGet release is published, install from the normal registry:

```sh
dotnet add package CoAkka.Runtime --version 1.3.4
```
...
4. source-checkout staging under `v2/staging/native/1.3.3+282f3ad/<platform>`
