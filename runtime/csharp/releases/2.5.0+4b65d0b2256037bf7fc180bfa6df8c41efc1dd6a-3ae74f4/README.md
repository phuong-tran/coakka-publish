...
This package targets `net8.0` and is tested in .NET 8, .NET 9, and .NET 10
applications. Kubernetes is supported but not required. Use
the public [Ecosystem Overview](https://github.com/phuong-tran/coakka-samples/blob/main/docs/ecosystem-overview.md)
...
verified before pack. Exact NuGet `CoAkka.Runtime==2.5.2` request/reply and
...
```text
csharp/build/nupkg/CoAkka.Runtime.2.5.2.nupkg
```
...
runtime. See the shared [file-lane contract](https://github.com/phuong-tran/coakka-samples/blob/main/docs/runtime-file-transfer.md).
...
`StreamLane.OpenOwned(...).PreparePublishGrant(...)`. Public grant constructors
support trusted control-plane deserialization; derive the remote job with
