...
Published module `v1.6.0` embeds connector version `2.3.0` and native runtime
generation `2.3.0+a83ab412`. The prepared `2.4.0` source archive instead uses
native generation `2.4.0+c2f53117`; its public module tag is assigned at module
publication. The module path has no `/v2` suffix, so module and runtime version
numbers intentionally remain separate.
...
The `2.4.0` source archive contains the Stream Lane connector surface over
exact native generation `2.4.0+c2f53117`. Keep that pairing intact and follow the public
