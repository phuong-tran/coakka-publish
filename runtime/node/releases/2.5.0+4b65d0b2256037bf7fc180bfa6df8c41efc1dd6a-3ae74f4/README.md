...
See the public [Ecosystem Overview](https://github.com/phuong-tran/coakka-samples/blob/main/docs/ecosystem-overview.md)
...
records exact package versions and separates payload verification from
matching-host execution. Exact npm `coakka-v2-connector-node@2.5.2`
request/reply passes on macOS ARM64 and Linux ARM64/x86-64.
...
Minimum host: Node.js 22. The package is also gated on the current Node.js LTS
line; the Node version used to build a release does not raise this floor.
...
See [Connection Strategies](https://github.com/phuong-tran/coakka-samples/blob/main/docs/connection-strategies.md),
...
path for blocking waits. Package `2.5.2` includes the File-Lane-capable native
runtime `2.5.0+4b65d0b2256037bf7fc180bfa6df8c41efc1dd6a`; see the shared [file-lane contract](https://github.com/phuong-tran/coakka-samples/blob/main/docs/runtime-file-transfer.md).
...
For replicas, use `FileLane.openOwned(...).prepareReceiveGrant(...)` or
`StreamLane.openOwned(...).preparePublishGrant(...)`. Grant constructors accept
the object shape returned by `toJSON()` at the remote end; derive the job with
`toSendSpec(...)` or `toSubscribeSpec(...)`. See the
