...
- release-candidate wheel `coakka-v2-connector==2.1.0` request/reply passes on macOS ARM64
  and Linux ARM64/x86-64
- the same 2.1.0 source uses `PeekNamedPipe` on Windows ARM64 and passes the
  request/reply smoke
...
| Linux ARM64 | Yes | Release-candidate wheel `2.1.0` request/reply passes |
| Linux x86-64 | Yes | Release-candidate wheel `2.1.0` request/reply passes |
| Windows ARM64 | Yes | Current 2.1.0 source request/reply passes with `PeekNamedPipe` readiness |
