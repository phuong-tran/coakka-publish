...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-0.2.0-spike" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-0.2.0-spike/Cargo.toml --bin coakka-rust-smoke
```
