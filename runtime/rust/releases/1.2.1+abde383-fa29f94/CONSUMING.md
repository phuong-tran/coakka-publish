...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-1.2.1-spike" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-1.2.1-spike/Cargo.toml --bin coakka-rust-smoke
```
