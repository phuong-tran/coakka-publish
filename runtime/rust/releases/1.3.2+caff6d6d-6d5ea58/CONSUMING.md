...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-1.3.2-spike" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-1.3.2-spike/Cargo.toml --bin coakka-rust-smoke
```
