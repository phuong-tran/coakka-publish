...
Source version `1.4.1` uses exact native generation
`1.4.1+9e02a51d`. It is a local source coordinate, not a crates.io release.
...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-1.4.1" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-1.4.1/Cargo.toml --bin coakka-rust-smoke
```
...
All five payloads must match the candidate ledger. Current execution evidence
