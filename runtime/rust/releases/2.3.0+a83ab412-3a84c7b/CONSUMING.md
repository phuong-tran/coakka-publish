...
Source version `2.3.0` uses exact native generation
`2.3.0+a83ab412`. It is a local source coordinate, not a crates.io release.
...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-2.3.0" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-2.3.0/Cargo.toml --bin coakka-rust-smoke
```
...
All five payloads must match the release ledger. Version 2.3.0 passes native
