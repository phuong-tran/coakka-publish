...
Source version `2.4.0` uses exact native generation
`2.4.0+c2f53117`. It is a local source coordinate, not a crates.io release.
...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-2.4.0" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-2.4.0/Cargo.toml --bin coakka-rust-smoke
```
...
All five payloads must match the release ledger. Version 2.4.0 passes native
