...
Source version `2.5.1` uses exact native generation
...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-2.5.1" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-2.5.1/Cargo.toml --bin coakka-rust-smoke
```
...
All five payloads must match the release ledger. Version 2.5.1 passes native
