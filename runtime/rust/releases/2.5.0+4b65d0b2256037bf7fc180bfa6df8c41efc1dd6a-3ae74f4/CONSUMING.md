...
Source version `2.5.2` uses exact native generation
`2.5.0+4b65d0b2256037bf7fc180bfa6df8c41efc1dd6a`. It is a local source coordinate, not a crates.io release.
...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-2.5.2" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-2.5.2/Cargo.toml --bin coakka-rust-smoke
```
...
All five payloads must match the release ledger. Version 2.5.2 passes native
