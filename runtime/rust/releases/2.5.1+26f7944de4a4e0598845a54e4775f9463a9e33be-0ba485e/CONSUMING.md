...
Source version `2.5.3` uses exact native generation
`2.5.1+26f7944de4a4e0598845a54e4775f9463a9e33be`. It is a local source coordinate, not a crates.io release.
...
```toml
[dependencies]
coakka-runtime-rs = { path = "/path/to/coakka-runtime-rs-2.5.3" }
```

Run the packaged smoke:

```sh
cargo run --manifest-path /path/to/coakka-runtime-rs-2.5.3/Cargo.toml --bin coakka-rust-smoke
```
...
All five payloads must match the release ledger. Version 2.5.3 passes native
...
execution record.
Use [Transport Configuration](TRANSPORT_CONFIGURATION.md) and [common troubleshooting](https://github.com/phuong-tran/coakka-publish/blob/main/docs/troubleshooting.md)
